...
levelSelect = False
gameEndScreen = False
gameEndScreen2 = False
toBeContinuedScreen = False
...
#RAT SPRITE IMAGES
for i in range(10):
    catPicList_R[i] = pygame.image.load("catWalk" + str(i) + ".png") 
    catPicList_L[i] = pygame.image.load("LcatWalk" + str(i) + ".png")
...
ratPicListR = [ratPicList[:3]] #the first 3 rat images are facing right
...
terrain3x2 = pygame.image.load('terrain(240x160).png') 
...
def grenadeCounter(grenadeNum): #(Rosie)
    if levelChosen == 'beach': #makes the text more visible depending on the level chosen
        colour = NAVY
    elif levelChosen == 'forest':
        colour = WHITE
    if grenadeNum == 3:
        gameWindow.blit(grenades3, (30, 30))
        instructions = nameFont.render('Press Space To Shoot Grenades', 1, colour)
...
class ratEnemy(pygame.sprite.Sprite):
    def __init__(self, ratX, ratY, speed, direction, ratNum): #setting parameters
        ...
        self.ind = ratNum
    def movement(self, ratLimitL, ratLimitR, scrollingDir): #setting the rat limit variables here, so it updates each loop
        self.ratNum = ratIndexList.index(self.ind) #the index number has to be updated since the items from the list are being deleted which affects the indexes (eg. when the cat kills the rat with the grenade)
...
#SHORT STARTING STORY: Katelyn --------------------------------------------------------------------------------------------------------------------------------------------------------------------

  while storyStart:
        pygame.event.clear()
        buttons = pygame.mouse.get_pressed()
        
        nezumiEscapeStartStory = pygame.image.load("nezumiEscapeStartStory.png")
        gameWindow.blit(nezumiEscapeStartStory, (0,0))

        if buttons[0]:
            mouseX, mouseY = pygame.mouse.get_pos()
            if mouseX in range(241, 548) and mouseY in range(480, 522):
                openingScreen = True
                storyStart = False
...
#GAME STARTING SCREEN: Katelyn ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if (openingScreen == True or levelSelect == True) and music == True:
  ...
  while openingScreen:
    ...
    for event in pygame.event.get(): #(Rosie)
...
#LEVEL SELECTION: Katelyn ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  if levelSelect == True and music == True:
    pygame.mixer.music.stop()
    pygame.mixer.init(44100, -16, 2, 2048)
    pygame.mixer.music.load("levelSelectMusic.ogg")
    pygame.mixer.music.set_volume(1)
    pygame.mixer.music.play(-1)
  elif music == False:
    pygame.mixer.music.stop()
...
# LEVELS SCREEN: Katelyn -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  if levelsScreen == True and music == True:
      pygame.mixer.music.stop()
      pygame.mixer.init(44100, -16, 2, 2048)
      pygame.mixer.music.set_volume(1)
      if levelChosen == "beach":
          pygame.mixer.music.load("beachMusic.ogg")
          pygame.mixer.music.play(-1)
...
#LEVEL VARIABLE RESET: Katelyn ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
    while levelReset == True:
      if levelChosen == "beach": #(Rosie)
        ...
        ratEnemyLimitsL = [[1355], [3776], [2855]] #controls when the rat changes directions
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        #RESETTING POWERUPS (Rosie)
        powerupRectList = [[1000, 213, 50, 50]]
        grenadeRectList = [] 
        grenadeNum = 3 
        grenadeList = [] 
        grenadeActivation = False
        ...
        deadlyObstaclesList = [[909, 286, 24, 34], [1337, 171, 24, 34], [2932, 172, 24, 34],]
        ...
        deadlyObtaclesSpriteList = [[889, 286, spikes2], [1317, 171, spikes2], [2912, 172, spikes2]]
...
#START OF PLAYING LEVEL ==================================================================================================================================================================
    ...
    if len(grenadeList) > 0:
        if rectCollide(pygame.Rect(grenadeRectList[0]), ratEnemyRectList) == True:
            ...
            del ratEnemyList[index] #deletes that rat enemy using the index from all of the following lists
    ...
    #IF CAT PICKS UP POWERCUBE (Rosie)
    if rectCollide(catRect, powerupRectList) == True:
        powerupRectList.remove(rectCollideIndexFinder(catRect, powerupRectList)) #stops displaying the sprite by finding that value in the list and removing it
    ...
    #SCROLLING BACKGROUND CHARACTER MOVES LEFT (Rosie)
        ...
        scrollingDir = 'right' #used for the rat function
    else:
        scrollingDir = ''
...
#END OF STORY: Katelyn -------------------------------------------------------------------------------------------------------------------------------------------------

  if gameEndScreen == True and music == True:
      pygame.mixer.music.stop()
      pygame.mixer.init(44100, -16, 2, 2048)
      pygame.mixer.music.set_volume(1)
      pygame.mixer.music.load("endMusic.ogg")
      pygame.mixer.music.play(-1)
  elif music == False:
      pygame.mixer.music.stop()
          
  while gameEndScreen:
      if gameEndReset == True:
          storyBoard = 1
          gameEndReset = False
      ...
      nezumiEscapeEnd1 = pygame.image.load("nezumiEscapeEnd1.png")
      nezumiEscapeEnd2 = pygame.image.load("nezumiEscapeEnd2.png")

      if storyBoard == 1:
          gameWindow.blit(nezumiEscapeEnd1, (0,0))
              
      if storyBoard == 2:
          gameWindow.blit(nezumiEscapeEnd2, (0,0))

      if buttons[0]:
          mouseX, mouseY = pygame.mouse.get_pos()
          if mouseX in range(363, 730) and mouseY in range(394, 482):
              storyBoard = 2
                  
      if storyBoard == 2 and buttons[0]:
          mouseX, mouseY = pygame.mouse.get_pos()
          if mouseX in range(275, 525) and mouseY in range(122, 139):
              gameEndScreen2 = True
              gameEndScreen = False
              gameEndReset = True
  ...
  while gameEndScreen2:
      ...
      catBoat = pygame.image.load("catBoat.png")
            
      gameWindow.blit(nezumiEscapeEndingScreen, (0,0))
      gameWindow.blit(catBoat, (boatX, boatY))
            
      if keys [pygame.K_d]:
            boatX += shift
      if boatX > 800:
            gameEndScreen2 = False
            toBeContinuedScreen = True
            gameEndReset == True
  ...
  if toBeContinuedScreen == True and music == True:
      pygame.mixer.music.stop()
      pygame.mixer.init(44100, -16, 2, 2048)
      pygame.mixer.music.set_volume(1)
  ...
  while toBeContinuedScreen:
      pygame.event.clear()
      keys = pygame.key.get_pressed()
      nezumiEscapeToBeContinued = pygame.image.load("nezumiEscapeToBeContinued.png")
      gameWindow.blit(nezumiEscapeToBeContinued, (0,0))
